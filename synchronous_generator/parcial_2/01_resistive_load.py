...
def main() -> None:
    ...
    ax.quiver(0, 0, v_phase_vec[0], v_phase_vec[1], angles="xy", scale_units="xy",
              scale=1, color="blue",
              label=fr"$V_{{\phi}}={{{phase_voltage}}}\; V \; \angle 0^{{\circ}}$")

    ax.quiver(0, 0, ia_vec[0], ia_vec[1], angles="xy", scale_units="xy",
              scale=0.04, color="red",
              label=fr"$I_A={{{ia_magnitude:.2f}}}\; A \; \angle {{{ia_phase_angle}}}^{{\circ}}$ (not to scale)")
    ...
    ax.set_ylim(min(0, ia_vec[1])-100, max(ea_vec[1], v_phase_vec[1])+100)
    plt.show()
