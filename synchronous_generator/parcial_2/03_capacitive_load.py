...
def main() -> None:
    ...
    h.label_vector(ax, origin, ia_vec, r"$I_A$", "red", offset_x=90, offset_y=40)
    ...
    h.label_vector(ax, v_xs_vec_start, v_xs_vec, r"$I_A jX_S$", "black", offset_x=130, offset_y=-20)

    ax.set_xlim(min(0, ea_vec[0])-200, max(ea_vec[0], v_phase_vec[0]+v_ra_vec[0])+300)
    ax.set_ylim(min(0, ia_vec[1])-300, max(ea_vec[1], v_phase_vec[1])+200)
    ax.set_aspect("equal")
