...
def main() -> None:
    ...
    for i in s_values:
        shaft_velocity = helpers_ex2.shaft_velocity(sync_velocity_rpm, i/50)
        torque_r2 = helpers_ex2.torque_induced(v_th, r_th, x_th, R2, X2, sync_velocity_radsec, i/50)
        torque_r2_doubled = helpers_ex2.torque_induced(v_th, r_th, x_th, r2_doubled, X2, sync_velocity_radsec, i/50)
    ...
    print(f"c). Si se duplica R2 (2R2: {r2_doubled} ohms), el deslizamiento se duplica, al cual se presenta el par máximo:")
    ...
    plt.plot(shaft_velocity_list, [torque_max]*50, '--', label=fr"$\tau_{{máx}}={torque_max:.2f}$ Nm", color='red')
